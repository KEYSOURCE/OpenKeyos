{assign var="paging_titles" value="KERM, Manage customer added users, Edit user"}
{assign var="paging_urls" value="/?cl=kerm, /?cl=kerm&op=customer_added_users"}
{include file="paging.html"}

<h1>Edit user {$aduser->first_name} {$aduser->last_name}</h1>
<p class="error">{$error_msg}</p>

<form name="frm_edit_usr" method="POST" action="">
{$form_redir}
<table class="list" width="98%">
	<thead>
		<tr>
			<td colspan=2>Edit user configuration</td>
		</tr>
	</thead>
	...
	<tr>
		...
		<td class="post_headlight">
			...
			{html_options options=$customers selected=$fc}
			</select>
		</td>
	</tr>
	{else}
	<tr>
		<td width="150px" class="headlight">Customer: </td>
		<td class="post_headlight">
		{assign var="cid" value=$aduser->customer_id}
		{$customers.$cid}
			<input type="hidden" name="aduser[customer_id]" value="{$cid}">
		</td>
	</tr>	
	{/if}
	<tr>
