...
<h1>Edit WebAccess Credentials</h1>
<p class="error">
    {$error_msg}
</p>
...
<table class='list' name="cred_tbl" id="cred_tbl">
    ...
    <tbody name="cred_list_body" id="cred_list_body">
        ...
        {foreach from=$webaccess->credentials item="cred"}
            <tr id='cred_row_{$cnt}'>
                <td>{$cred->username}</td>
                <td>{$cred->password}</td>
                <td>{$cred->notes}</td>
                <td><a id='edt_{$cnt}' href='#' onclick='edit_credential_row(this, {$cnt})'>Edit</a></td>
                <td><a id='edt_{$cnt}' href='#' onclick='delete_credential_row(this,{$cnt})'>Delete</a></td>
            </tr>
...
<div name="hid_inputs" id="hid_inputs" style="display: block;" >
    {assign var="cnt" value="0"}
    {foreach from=$webaccess->credentials item="cred"}
        <div id="usr_in_{$cnt}">
            <input type='hidden' name='webaccess[credentials][{$cnt}][username]' value="{$cred->username}" />
        </div>
        <div id="pass_in_{$cnt}">
            <input type='hidden' name='webaccess[credentials][{$cnt}][password]' value="{$cred->password}" />
        </div>
        <div id="notes_in_{$cnt}">
            <input type='hidden' name='webaccess[credentials][{$cnt}][notes]' value="{$cred->notes}" />
        </div>
        {assign var="cnt" value=$cnt+1}
    {/foreach}
</div>
