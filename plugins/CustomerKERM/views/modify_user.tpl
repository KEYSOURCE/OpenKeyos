{assign var="paging_titles" value="Technical Support, Manage users, Modify user"}
{assign var="paging_urls" value="/?cl=customer_kerm&op=manage_users, /?cl=customer_kerm&op=modify_user"}
{include file="paging.html"}

<h1>Modify exchange user</h1>
<p class="error">{$error_msg}</p>

<form name="frm_modi_usr" method="POST" action="">
{$form_redir}
<table class="list" width="98%">
	...
	<tr>
		...
		<td class="post_headlight">
			<select id="aduser[customer_id]" name="aduser[customer_id]" onchange="document.forms.frm_modi_usr.submit();">
	...
	<tr>
		<td width="150px" class="headlight">Customer:  </td>
		<td class="post_headlight">
		{foreach from=$customers_list item=customer key=k}
		{$customer}
...
<table class="list" width="98%">
	...
	<tr>
		...
		<td  class="post_highlight">
		{assign var="grp" value=$aduser->get_group()}
		{assign var="cidx" value=$aduser->customer_id}
		<select name="aduser[GroupName]" id="aduser[GroupName]">
		{html_options options=$groups_list.$cidx selected=$grp}
		</select>
		</td>
	</tr>
	<tr>
