{assign var="paging_titles" value="Customers, Manage Customers Photos, Add Photo"}
...
<table class="list" width="70%">
	...
	<tr>
		<td>Subject:</td>
		<td colspan="2">
			<input type="text" name="photo[subject]" value="{$photo->subject|escape}" size="40">
		</tr>
	</tr>
	<tr>
		<td>External URL:</td>
	...
	<tr>
		<td>Comments:</td>
		<td colspan="2">
			<textarea name="photo[comments]" rows="5" cols="40">{$photo->comments|escape}</textarea>
		</td>
	</tr>
	<tr>
		<td>Linked to:</td>
...
{if $photo->get_full_path()}
	Image size: {$photo->width} x {$photo->height}<br/>
	<p>{$photo->get_tag()}</p>
{else}
	<p>[Image not available]</p>
{/if}