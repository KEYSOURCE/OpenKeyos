{assign var="paging_titles" value="KAWACS, Manage monitor items, Edit item, Edit field "}
...
<h1>Edit Monitoring Item Field</h1>
...
<table width="98%" class="list">
	...
	<tr>
		...
		<td class="post_highlight"><b>{$parent_item->id} : {$parent_item->name}</b></td>
	</tr>
	...
	<tr id="lists_row" style="display:none;">
		...
		<td class="post_highlight">
			...
			<div id="div_list_values_{$list_id}" style="display:none;">
				<b>Values:</b><br/>
				{assign var="max_first" value=10}
				{assign var="max_last" value=10}
				{assign var="separ_shown" value=0}
				
				{foreach from=$list_values key=k item=v name=list_values}
					{assign var="iter" value=$smarty.foreach.list_values.iteration}
					{if $iter<=$max_first or $iter>=(count($list_values)-$max_last)}
						&nbsp;&nbsp;&nbsp;{$k|escape} : {$v|escape}<br/>
					{elseif !$separ_shown}
						{assign var="separ_shown" value=1}
						&nbsp;&nbsp;&nbsp;<font class="light_text">[ . . . there are {$list_values|@count} values in total . . . ]</font><br/>
					{/if}
				{/foreach}
			</div>
			{/foreach}
